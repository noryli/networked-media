...
/** Updates theme every Monday at midnight */
function updateTheme() {
    ...
    const now = new Date();
    if (themeData.lastUpdated !== getWeekNumber()) {
        themeData = { lastUpdated: getWeekNumber(), theme: themes[Math.floor(Math.random() * themes.length)] };
        fs.writeFileSync(themeFile, JSON.stringify(themeData, null, 2));
        fs.writeFileSync(catsFile, "[]", "utf8"); // Clears previous data
        console.log("Theme updated and data cleared for new week.");
    }
    return themeData.theme;
}

/** Sets interval to check for weekly reset */
setInterval(() => {
    const now = new Date();
